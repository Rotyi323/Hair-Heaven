...
if ($mysqli && $_SERVER['REQUEST_METHOD'] === 'POST' && !$flash['err']) {
  try {
    ...
    if (post('action') === 'product_delete') {
      ...
      if ($id > 0) {
        ...
        if ($st->affected_rows > 0) {
          ...
          $flash['err'] = 'A termék nem törölhető.';
      }
    }
    ...
    if (post('action') === 'service_save') {
      ...
      if ($name === '' || $minutes <= 0 || $price < 0)
        throw new RuntimeException('Hiányzó vagy hibás mezők.');
    ...
    if (post('action') === 'service_delete') {
      ...
      if ($id > 0) {
        ...
        $st->execute();
        if ($st->affected_rows > 0) {
          $flash['ok'] = 'Szolgáltatás törölve.';
          audit($mysqli, $userId, 'delete', 'services', $id);
        } else
          $flash['err'] = 'A szolgáltatás nem törölhető.';
      }
    }
  } catch (Throwable $ex) {
...
<head>
  ...
  <style>
    ...
    .switch-wrap .form-check-input:focus {
      box-shadow: 0 0 0 .15rem rgba(199, 109, 240, .25);
    }
...
<body>
  ...
  <div class="container-xxl py-4">
    ...
    <?php if ($flash['ok']): ?>
      <div class="alert alert-success"><?= e($flash['ok']) ?></div><?php endif; ?>
    <?php if ($flash['err']): ?>
      <div class="alert alert-danger"><?= e($flash['err']) ?></div><?php endif; ?>

    <ul class="nav admin-tabs" role="tablist">
      <li class="nav-item" role="presentation">
        <button class="nav-link active" data-bs-toggle="tab" data-bs-target="#tabProducts" type="button"
          role="tab">Termékek</button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="nav-link" data-bs-toggle="tab" data-bs-target="#tabServices" type="button"
          role="tab">Szolgáltatások</button>
      </li>
      <li class="nav-item" role="presentation">
                  ...
                  <tbody>
                    <?php foreach ($products as $p): ?>
                      <tr data-id="<?= (int) $p['id'] ?>">
                        ...
                        <td>
                          <span
                            class="badge <?= $p['is_active'] ? 'badge-on' : 'badge-off' ?>"><?= $p['is_active'] ? 'Aktív' : 'Inaktív' ?></span>
                          <?php if ($p['is_featured']): ?><span
                              class="badge text-bg-warning">Kiemelt</span><?php endif; ?>
                        </td>
                        <td class="text-end">
                          <div class="d-inline-flex gap-2">
                            <button class="btn btn-sm btn-outline-primary js-edit-product" title="Szerkesztés"
                            ...
                            <form method="post" class="d-inline js-del-product"
                              ...
                              <button class="btn btn-sm btn-outline-danger" title="Törlés"><i
                    ...
                    <?php if (empty($products)): ?>
                      <tr>
                        <td colspan="7" class="text-muted">Nincs termék.</td>
                      </tr>
                    <?php endif; ?>
                  </tbody>
                </table>
                ...
                <div class="row g-4">
                  <div class="col-md-6">
                    ...
                    <div class="d-flex gap-2 mt-3">
                      <button class="btn btn-primary" type="submit"><i class="fa-solid fa-floppy-disk me-1"></i>
                        Mentés</button>
                      <button class="btn btn-outline-secondary" type="button" id="btnResetProduct"><i
                          class="fa-solid fa-rotate-left me-1"></i> Újrakezdem</button>
                    </div>
                  </div>
                  ...
                  <tbody>
                    <?php foreach ($services as $s): ?>
                      <tr data-id="<?= (int) $s['id'] ?>">
                        ...
                        <td><span
                            class="badge <?= $s['is_active'] ? 'badge-on' : 'badge-off' ?>"><?= $s['is_active'] ? 'Aktív' : 'Inaktív' ?></span>
                        </td>
                        <td class="text-end">
                          <div class="d-inline-flex gap-2">
                            <button class="btn btn-sm btn-outline-primary js-edit-service" title="Szerkesztés"
                    ...
                    <?php if (empty($services)): ?>
                      <tr>
                        <td colspan="6" class="text-muted">Nincs szolgáltatás.</td>
                      </tr>
                    <?php endif; ?>
                  </tbody>
                </table>
            ...
            <div class="card p-3">
              ...
              <form method="post" id="serviceForm">
                ...
                <div class="d-flex gap-2">
                  <button class="btn btn-primary" type="submit"><i class="fa-solid fa-floppy-disk me-1"></i>
                    Mentés</button>
                  <button class="btn btn-outline-secondary" type="button" id="btnResetService"><i
                      class="fa-solid fa-rotate-left me-1"></i> Újrakezdem</button>
                </div>
              </form>
            </div>
