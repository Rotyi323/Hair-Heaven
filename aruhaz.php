...
if (!$mysqli || empty($brands)) {
  $brands = ["Garnier", "Schwarzkopf", "L'Oréal", "Kérastase"];
}

//Ár csúszka tartománya
$rangeMin = 0;
$rangeMax = 100000;
...
//részleges egyezés
  ...
  $items = array_filter($all, function ($p) use ($q, $brand, $type, $curMin, $curMax) {
    if ($brand !== '' && strcasecmp($p['brand'], $brand) !== 0)
      return false;
    if ($type !== '' && $p['type'] !== $type)
      return false;
    if ($q !== '') {
    ...
    if ($p['price'] < (float) $curMin)
      return false;
    if ($p['price'] > (float) $curMax)
      return false;
    return true;
  });
...
//Segéd: query string építő lapozáshoz 
...
<body>
  ...
  <div class="container mt-4">
    <div class="d-flex align-items-end justify-content-between flex-wrap gap-2">
      <div>
        ...
        <div class="text-muted">Válogass prémium hajápolási termékeink közül – szűrés és rendezés egy kattintással.
        </div>
      </div>
    </div>
  </div>
        ...
        <div class="col-12">
          ...
          <div class="range-wrap">
            <div class="multi-range" data-min="<?= (int) $rangeMin ?>" data-max="<?= (int) $rangeMax ?>">
              ...
              <input type="range" id="rangeMin" min="<?= (int) $rangeMin ?>" max="<?= (int) $rangeMax ?>" step="10"
                value="<?= (int) $curMin ?>">
              <input type="range" id="rangeMax" min="<?= (int) $rangeMin ?>" max="<?= (int) $rangeMax ?>" step="10"
                value="<?= (int) $curMax ?>">
            </div>
            <div class="range-values">
              <div class="input-group">
                <span class="input-group-text">Min</span>
                <input type="text" inputmode="numeric" pattern="[0-9 ]*" class="form-control price-input" id="min_price"
                  name="min_price" value="<?= number_format((int) $curMin, 0, ',', ' ') ?>">
                <span class="input-group-text">Ft</span>
              </div>
              <div class="input-group">
                <span class="input-group-text">Max</span>
                <input type="text" inputmode="numeric" pattern="[0-9 ]*" class="form-control price-input" id="max_price"
                  name="max_price" value="<?= number_format((int) $curMax, 0, ',', ' ') ?>">
                <span class="input-group-text">Ft</span>
              </div>
              <small class="text-muted ms-2">Tartomány: <?= number_format($rangeMin, 0, ',', ' ') ?> –
                <?= number_format($rangeMax, 0, ',', ' ') ?> Ft</small>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="container my-4">
    <div class="row g-4">
      <?php if (empty($items)): ?>
        <div class="col-12">
          <div class="alert alert-secondary">Nincs a szűrésnek megfelelő termék.</div>
        </div>
      <?php else: ?>
